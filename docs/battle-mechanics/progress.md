...
### 2026-09-12 · 第四次会话 · 追加：glossary.md 战斗关键字词典填完

- 用户："提交完继续往后做"
- 提交 commit `766105b`（08 章 + 07 纠错）已推 origin/master
- 剩余任务列表：glossary / 附录 / debug log 校准 —— 挑最独立可闭环的 glossary
- 方法：用 python 脚本读 `BattleKeywordDes.json` 和 `zh-CN.json` 拼出 53 条关键字全表（写到临时文件避开 bash 中文编码坑），然后**智能分类**成 10 大类而不是简单表格 dump
- 写完 **glossary.md** 从占位 45 行 → 完整 200+ 行
- **10 大类分组**：
  1. 异常状态类 debuff（燃烧/寄生/麻痹/中毒/疾速/冻伤/重量 —— 7 条 buff 承载）
  2. 增益状态类（湿润/光环/月轮/血棘/糖衣 —— 5 条）
  3. 天气 / 场地（9 条，MagicEnvir 承载；发现 8 种场地覆盖全部 15 种属性的对称设计）
  4. 陷阱（荆棘/毒潭/三角沙 —— 3 条 Trap 承载，跟 20s 换宠 CD 联动特别贵）
  5. 印记（光/影/光环/月轮 —— 光暗牵制设计）
  6. 攻防等级（8/9，用 `(5+n)/5` 反推填的档位式 buff）
  7. 暴击/伤害加成/伤害减免（结算修正类）
  8. 其他机制（入梦/光能/贯穿/修炼/强化/愿望）
  9. 专属机制（史莱姆之力/暗夜之拥/蜇伤/恒星/星辉/葱鸡/草莓/燃烧汲取/毒素汲取 —— 特定 lumi）
  10. 系统概念（初训/竭力技能 —— 竭力=LifeSkill 同一个东西的两种表达）
- **每条关键字加**：
  - Id + emoji + 中文名 + 描述
  - **服务端承载子系统**（buff / trap / magicenvir / etc.）+ 关键代码引用
  - **分析价值**（这个关键字对平衡分析有什么用）
  - **跟其他关键字的对称/关联设计**（比如燃烧↔中毒是"挨打受伤 vs 打人受伤"）
- **新发现**：
  - Id=5 缺失（1→2→3→4→6 跳过 5）—— 又一个 [[battle-design-evolution]] 案例（已加 unknown P2）
  - #19 光能描述"消耗 30 点光能"跟 TrainerSkill.ChargeNum=300 不符 —— 10 倍单位换算差异？多语言文档滞后？（已加 unknown P2）
  - #39 伤害减免的描述文案"提升"是错字（应该是"降低"，公式里明明是减号）—— 描述文案 bug
- **10 大类跨章速查表**：给每个关键字对应到 [[04/05/06]] 主章节 —— 读者点关键字能秒定位服务端逻辑

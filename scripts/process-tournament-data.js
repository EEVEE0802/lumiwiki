...
// 天梯数据路径（用于获取玩家段位）
const ladderPaths = week
  ? weekDates.map(d => path.join(__dirname, `../data/${region}/archive/daily/ladder/${d}.csv`))
...
async function processTournamentData() {
  ...
  try {
    await processCSVStream(ladderPaths, (row) => {
  ...
  await processCSVStream(tournamentPaths, (row) => {
    const playerType = parseInt(row.player_type)
